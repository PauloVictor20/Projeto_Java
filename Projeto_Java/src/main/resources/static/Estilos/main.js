...
const saveClient = () => {
    ...
    if (isValidFields()) {
        const client = {
            ...
            descriçao: document.getElementById('descriçao').value
...
const fillFields = (client) => {
    ...
    document.getElementById('preço').value = client.preço
    document.getElementById('tamanho').value = client.tamanho
    document.getElementById('genero').value = client.genero
    document.getElementById('descriçao').value = client.descriçao
...
const editDelete = (event) => {
    if (event.target.type == 'button') {
        ...
        if (action == 'edit') {
            ...
            avisoDelete.textContent = `Deseja realmente excluir o sapato ${client.nome}?`
            openModal2()
...
const botao = document.getElementById("card-title1")

botao.addEventListener("change", nometitulo)

function nometitulo(client){
    document.getElementById('nome').value = client.nome
